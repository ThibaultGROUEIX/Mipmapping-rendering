...
{   
    ...
    glBindFramebuffer(GL_FRAMEBUFFER,handleFBOStart);
    // |= check//("glBindFramebuffer(FBO)");
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, handleTextures[i-1], 0);
    // |= check//("glFramebufferTexture2D");
	

    GLuint idNewTexture;
	MipMap::initTextureColor(&idNewTexture, width/(pow(2,i)), height/(pow(2,i)));
